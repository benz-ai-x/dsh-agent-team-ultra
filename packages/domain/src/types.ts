...
export type ProfileJsonValue = null | boolean | number | string | readonly ProfileJsonValue[]
  | { readonly [key: string]: ProfileJsonValue }

export interface ProfileTextBlock {
  /** Preserve unknown wire fields so Host strict validation can reject them instead of silently dropping them. */
  readonly [field: string]: ProfileJsonValue
  readonly id: string
...
/** One caller-owned, Team-scoped UUID for a launch decision. */
export type LaunchRequestId = string
...
export interface DigitalEmployeeInstanceView {
  ...
  readonly actualRoute?: { readonly provider: string; readonly model: string; readonly reasoningEffort?: string }
