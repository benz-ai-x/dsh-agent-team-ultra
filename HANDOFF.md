...
- 当前主工作区分支为 `fix/31-conversation-profile-launch`，从最新 main `1a74d7fe28b11b8c014e648c9bb43bf246112539` 建立。#27–#30 已完成并分别合并 PR #53–#56，Issue 已关闭，飞书完成通知均成功。#31 已开始，目标是在用户明确要求 Team 协作时，让 Lead 通过明确的对话工具读取 Profile／Active Revision 并沿 Studio 同一 Host 流程幂等启动员工。
- 冻结清单的恢复状态在 `/root/workspace/.ultra-checks/frozen-issues-27-44.json`；Issue #31 的权威正文仍以 GitHub 为准。#39–#43 要求共用集成分支、最终门禁后合并，与本轮逐项合并 main 的顺序冲突；已经请求用户决策，相关操作保持待定，不影响先完成 #31–#38。
...
- **#31 建 PR 前候选完成（2026-09-07，Asia/Shanghai）**：Ultra 位于 `fix/31-conversation-profile-launch`，固定起点／origin/main 为 `1a74d7fe28b11b8c014e648c9bb43bf246112539`。三个固定 Lead 对话工具列出／读取既有 Profile 并沿 Studio 同一 Host 流程启动 Active Revision；普通 teammate 由自身作用域显式屏蔽，Evaluation Worker 在发布前排除，名称冲突拒绝启动并回滚。
...
- [#31 验收证据](docs/evidence/issue-31-acceptance.md)映射全部六项 AC；[ADR 0022](docs/adr/0022-launch-active-profiles-from-lead-conversations.md)记录权限、身份和复用边界。完整 `pnpm verify` 通过 **562 strict／0 警告、336 tests／29 files**，生成全部产物；八归档安装校验打包 Host 的固定名称和真实 Profile 组合，Web、Codex／Claude JSON／SQLite 新建恢复及完整卸载均成功。固定历史 Codex `debde06c`／Claude `081357d` 归档升级保留 Profile Revision、成员、native handle／session，并在升级后实际取得三个工具；维护 fork 与官方 `d347e7` 的 11 组 Team 公共行为对照和拒绝准入通过。历史升级首次预检发现旧 Harness 工作区依赖链接未恢复；按旧 lock 离线恢复后原命令自然通过，三个源码工作树保持干净。下一步完成提交前差异审查，提交／推送并创建 PR，之后执行固定 main 起点的 Standards／Spec 双轴评审。
...
- frozen manifest 当前为 #27–#30 complete、#31 in_progress、currentIssue 31；原 18 项范围和顺序不变。
