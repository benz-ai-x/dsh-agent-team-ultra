...
- 当前主工作区分支为 `feat/batch-2-task-dag-live`，从最新 main `2c5a355deefcf3c9dfc3384787e9cfe3de4678e3` 建立。Batch 1 的 [PR #59](https://github.com/benz-ai-x/dsh-agent-team-ultra/pull/59) 已按人工确认 merge commit 合并，#33 自动关闭；合并后 `pnpm verify` 再次通过 582 strict checks、346 tests 与完整八归档/Web/恢复/卸载，飞书【仅知会】发送成功。
...
- 阶段 A 的既有开发和验证记录保留；PR #45–#52 已合并，#27–#33 也已通过各自复审、验证和合并流程进入 main `2c5a355deefcf3c9dfc3384787e9cfe3de4678e3`。Batch 2 的 #34–#36 review 2修复和最终 gates 已完成，等待实时 Issue 验收勾选、PR正文与固定起点review 3；#37+ 不在本 Batch。阶段 C 的 Harness 集成、锁定和真实 native 验收仍需完成，父 Spec #18 保持 open。
...
- #35 在同一面板加入从当前权威 tasks 派生的原生 task-id checkbox。创建提交完整依赖；编辑开始即钉住 `expectedRevision`，随后一次 Host `edit` 原子提交正文、scope 和完整依赖，即使 watch 已刷新 selected task 也不能绕过 CAS。missing reference、非 owner、过期 Lead、跨 Team、自环和间接环均在无 event/revision 副作用下拒绝；A/B/C 场景只有 A/B 全完成后 C 才 ready/claimable。CAS 冲突与 watch trailing read 竞态时仍重读权威任务并保留明确未保存的正文／依赖草稿，英文和中文都不自动重试。
- 维护 Harness 分支最终head `503ad563ff226c2afc77608c432af3a80aae3279` 已推送到 fork；工作树 clean。Host、TeamAction 和 browser Slot 为 3 files／109 tests，built Remote、定向 Agent Team keyless replay、typecheck、GUI与32／32 docs gates通过。#35追加提交`07bbff8a1f`让production Browser消费generated `getTask`，任务从权威list消失后保留原selected id并展示只读deleted tombstone，切Team/服务代际时拒绝迟到详情。Ultra 已精确 prepare/install/build并通过strict 582／0；最终八归档真实覆盖 dependency preview、单 revision edge replacement、英中 stale-CAS 草稿/no-retry，且消息恢复、Web、双 native JSON/SQLite、registration release 与 uninstall 同时全绿。逐项证据见 [#35 验收证据](docs/evidence/issue-35-acceptance.md)。
- #36 Harness review-fix head `503ad563ff226c2afc77608c432af3a80aae3279` 已精确 HTTPS 推送并以 `ls-remote` 核对。公开 `agentTeams/watch` catalog现由正式 generator收录；TeamAction 对 authority reload 使用单一 in-flight/dirty queue，burst期间最多一个请求且只追加一次 trailing read。production `getTask`在任务被权威列表删除后读取同一selected id的只读墓碑，并以Team/service/detail generation拒绝迟到结果。TeamAction与Browser各自的Client context owner会等待所有React触发的Remote stream close静止；deferred transport、失败记录和service replacement均有真实Fiber测试。正式 keyless Web corpus收录同一公开面板的list与真实task-1→task-2 DAG；中文 stale统一为“陈旧”。
- Ultra 消息 child Slot 通过 stock Gateway snapshot stream共享该 committed fact：page replacement进行中拒绝并禁用旧 committed cursor append；message page与roster各自以in-flight/latest-dirty latch合并burst，filter变更会立即fence旧代际并串行一次trailing read，Team/service cleanup释放队列。same-Team service refresh保留未保存草稿，pending send中断后保留exact request并转unknown，重连／重挂载不自动发送。React cleanup立即且幂等触发watch dispose，Client context/Fiber owner等待live/replaced controls真正静止，错误交给Cordis lifecycle边界。review修复的message/mount owning files为25/25全绿；最终full gate通过582 strict／0警告、30 files／359 tests及8归档。精确结果只以 [pipeline state](PIPELINE_STATE.md) 为准。
