...
- 当前主工作区分支为 `fix/32-team-message-center`，从最新 main `5ab1c5a2c5e16b6832edbdef8dd1473bc913e022` 建立。#27–#31 已完成并分别合并 PR #53–#57，Issue 已关闭，飞书完成通知均成功。#32 已开始，目标是从现有 Agent Teams 入口提供一个公开组合、可分页筛选且如实显示投递状态的持久消息中心。
- 冻结清单的恢复状态在 `/root/workspace/.ultra-checks/frozen-issues-27-44.json`；Issue #32 的权威正文仍以 GitHub 为准。#39–#43 要求共用集成分支、最终门禁后合并，与本轮逐项合并 main 的顺序冲突；已经请求用户决策，相关操作保持待定，不影响先完成 #32–#38。
...
- TDD RED 为 Harness 8 项缺失行为和 Ultra 缺少 UI。最终 Harness 两个 owner 包 **269 tests／10 files** 通过，新 reader 单文件语句／分支／函数／行均 100%；built-lib Remote、真实 browser composition、keyless replay、Host／Client build 和 32／32 doc-sync 通过。Ultra focused 4 项、兼容补修 28 项和完整 **338 tests／30 files** 通过；`pnpm verify` 同时通过 **582 strict／0 警告**、八归档实际安装、Codex／Claude JSON＋SQLite 新建／恢复、真实 Web 和完整卸载。历史 Codex `debde06c`／Claude `081357d` 归档升级保留原 Revision、member、native handle 和单一 native session；维护 `a1342a7` 与官方 `d347e7` 的 11 组公共契约对照通过。旧 Harness 在前身归档卸载后缺失 hoisted brand→Cordis 链接，按原 frozen workspace 解析恢复；三个历史源码工作树保持干净。逐项记录见 [#32 验收证据](docs/evidence/issue-32-acceptance.md)。当前 GitHub Issue 仍 open、六项 AC 尚未勾选；下一步是文档校验、提交／推送、创建 PR，并对固定精确 head 运行 Standards／Spec 独立审查。
