...
- #41 实现与定向验证完成，正在提交发布；Issue AC 尚未更新、未发送完成通知。`issue41-focused-owner.log`：70／70、5 文件（audit 37、execution 24、相关公开业务／Profile 9）。实际 B 3 条压缩 Team／child 日志经真实相邻 codec；JSON／SQLite 源 bytes 不变，私有转换、pending 隔离、原身份恢复、暂停／临时前缀恢复／确定性复用／分歧拒绝、checkpoint 与 DSH/native Run Index 重建、Eval 历史与旧 Gate 失效均通过。完整历史归档与每个写入／标记前后矩阵仍属 #43；B 黄金输入不含 Ultra Binding 或 native，不夸大为完整历史 native 证明。
- #41 Profile `data` 公开入口联合检查实际 Session／storage roots 后才注册 Persistence、JSON／SQLite 与 Domain，同一 Fiber 释放。checked CLI 拒绝 pending；真实 Loader 测试覆盖 pending／混合根和两种后端生命周期。Harness 候选未改；offline install、完整 build、最终 strict 648／0 warnings 通过。运维入口 `pnpm migration:execute`；JSON 目标为 `target/storage` 而非默认 `storages`，README 明确双路径配置、源停写、SDK 自有历史／认证不复制及 Linux 限制。`codebase-design` 收拢联合入口，`domain-modeling` 记录 ADR 0027。下步发布 #41 后继续 #42，再 #43 完整门禁；未正式 review／合并，#44 未认证验收。
