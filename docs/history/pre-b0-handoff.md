...
此文件于 2026-09-09 归档，只保留历史事实，不是当前执行指令。
当前范围以根目录 TODO.md、HANDOFF.md 和 ADR 0028 为准。
下文相对路径仍按原根目录文档理解；已退役代码应从旧提交 f84584d 查阅。
...
## 当前追加：移除过时的 dsh-plugin-dev 技能

- 用户明确要求删除 `dsh-plugin-dev`，认为它已不适合使用。已从 `/root/.agents/skills` 和 `/root/.claude/skills` 移除两个同名启用软链接；`/root/.codex/skills` 没有另一份同名入口。原技能源码仓库 `/root/workspace/dsh-plugin-dev` 保留，没有删除其他项目。
- 两个软链接可从 `/root/.local/share/Trash/files/dsh-plugin-dev-removed.PDtVxq` 恢复；该目录不在技能发现路径内。仓库 AGENTS 已取消强制使用旧技能，改为核对所选 DSH 源码的公开 API 与 owning tests；`CLAUDE.md → AGENTS.md` 相对链接保留。
- 以后不再使用该技能。下方及既有研究／验收记录中的使用说明仅为历史事实，不是重新启用的指令；没有改写历史证据，也没有重新开始开发、修改 Harness／锁／运行数据或提交／推送。
- 移除前严格检查 648 PASS／0 warnings，日志 `/tmp/ultra-remove-dsh-plugin-dev-strict.log`。本轮使用 OpenAI Docs 核对本地技能发现机制，用 `writing-for-agents` 清理过时的强制触发规则。保留上一轮全部未提交研究／交接改动。

## 官方 0.1.5 重新评估（研究已完成，开发继续暂停）

- 用户于 2026-09-09 明确暂停开发，要求重新分析 `/root/workspace/deepseek-harness`，说明官方已做／未做、Ultra 的机会、已开发能力迁移和未开发工作重规划。本轮只做源码研究与本地研究／交接文档；此前持续开发、合并与通知的历史授权不能用于继续旧流水线。不改运行时代码、Issue／PR、锁、依赖、源码链接或用户数据，不提交／推送。
- 官方分析快照为 `5dda764ed3aa172535a7967b06ff95d9cbfe536a`、`0.1.5-alpha.1`。该目录有十个既有未提交的构建／文档相关文件；使用提交内容区分官方与本地改动，全部保留，未 build／install 或切分支。官方版本字符串不构成 Ultra 兼容资格。
- PR #63 已于 `2026-09-09T05:30:02Z` 合并为 `f84584def627b4af78a029af8788de73da0ad567`，main／origin/main 同步，本轮开始时干净。运行锁仍为维护 C `3c38b1d4e8bf219750203e44b1df033ced754e92`／`agent-team-ultra.phase-c.v1`／Session 2；本轮 `pnpm context:check:strict` 为 648 PASS／0 warnings，日志 `/tmp/ultra-official-015-research-strict.log`。历史完整 452 tests 与归档门禁见上一轮证据，不冒充本轮重跑。
- GitHub 实时回读：#19–#43 全部 CLOSED；父 Spec #18 和真实认证验收 #44 OPEN。未完成的 #44 不是“没有代码”，也不能由受控 native 自动化替代；暂停期间不执行真实账号旅程或修改验收状态。
- 研究入口为 [官方 0.1.5 重新评估](docs/research/2026-09-09-official-015-reevaluation.md)，详细依据分别在 [Team／native](docs/research/2026-09-09-official-015-team-native.md)、[格式与接口迁移](docs/research/2026-09-09-official-015-migration.md)、[Profile／产品边界](docs/research/2026-09-09-official-015-product.md)。普通 Team、Preset、Skills、审批等官方基础不能冒称新近取代 Ultra；持久 native 队友、员工发布／评测链与协作 UI 仍须逐项保留或显式减范围。
- 研究完成：一个无存储／无模型的四输入小实验确认官方 V2→V3 admission 对基础 Team payload 2 和两类维护必需事件的拒绝；没有运行完整官方 reader 或真实迁移。六份本地文档的 96 个本地链接、238 个固定源码引用／180 个文件与空白检查通过。本轮文档保持未提交，产品及锁未改。
- 已异步询问历史数据保留范围和维护方向，尚未获得选择。决策前保护全部既有数据、成员身份与当前已验收版本；不据此擅自执行迁移。新 Session 3、维护扩展必需事实、持久化发布时机及包发布状态须重新资格验证。
- 本轮使用 `research` 按技能要求分工做三项源码研究，主代理用 `dsh-plugin-dev` 核对公开契约／来源与升级边界、用 `codebase-design` 分析哪些职责复用官方、哪些保留在 Ultra；不另建第二套 Team 权威或为升级泛化多层适配。原 worktree、四个 stash 与独立 Git 管理修复目录均保留。

## PR #63 联合验收与条件合并（历史记录，已合并）
...
- 仅在上述候选完整门禁通过后，支持标签提升为 `agent-team-ultra.phase-c.v1`。维护 C `3c38b1d4e8`、官方 `d347e703`、格式、SDK／payload 不变；只读审计公开 RED → GREEN 区分源码资格与数据 complete。旧 candidate 目标不能跨资格直接复用或手改 manifest，须保留最新业务事实并迁移到新目标。最终新资格 `54280d1` 已通过全部三组历史升级和官方对照；首轮全量的 pack 出现受控 MCP 60000ms 超时，保持代码／断言／超时不变，独立完整重跑 452 tests／39 files 与全部归档门禁自然退出 0。失败日志保留，不声称确定或修复超时原因。完整支持资格候选 Standards／Spec 各 0；后续仅收口并复核说明文档，按授权正常发布／合并。
...
- 仅 Batch 4 隔离树锁定并准备干净候选 `3c38b1d4e8`，标签 `phase-c.integration-candidate.v1` 不宣称最终资格。main、PR #62、原 B 源码和链接保持原发布线。#39 代码 `49d0366` 已推送草稿 PR #63（base 为 PR #62 分支），Issue OPEN／4/5 AC，AC 2 等待 #43。飞书通知已于 21:22:45 成功发送，幂等键 `ultra-39-49d0366-focused`，不要重发。
...
- #43 运行时与验证修复已提交为 `35f050ca1587e68724deed4717c793f90f661185`，正常推送 PR #63 并回读远端一致；Issue／PR 正文已更新验收进度，仍 OPEN／DRAFT，AC 勾选、标题、标签和指派均保留，未发送 #43 完成通知。归档集合改为实际 Profile 入口与本地依赖闭包推导（当前 8 包，不固定数量），包含 `/data` 普通解析；所有非归档 Harness 依赖来自同一构建证明。真实安装曾发现内部 registry `dsh-brand 0.1.3-alpha.2` 与锁定 alpha.1 不符，现由显式 `--lock-local-peers` 约束顶层／嵌套依赖，固定 profile pnpm 11.7.0，并直接链接已核验 SDK／平台载荷；不放宽预检、不更换 SDK 或复制认证。冲突 override 与未核验来源在安装前拒绝，已有配置保留。
...
- #43 历史 Codex（`debde06`）／Claude（`081357d`）前身、以及 B 发布候选 PR #62（`4cecfe2`）均已真实归档创建 JSON／SQLite 员工，调用运维入口发布隔离目标，再以新归档的实际 `/data` Loader 恢复原 member／Revision／native handle 并续跑。B 双 native 完整查询、任务回执、原生操作、源保护、迁移后 Web 和无强杀退出、卸载通过：`issue43-b-direct-product-upgrade.log`。最终脚本、固定安装工具链及 SDK 链接已复验通过，历史最终证据为 `issue43-final-codex-history.log`／`issue43-final-claude-history.log`。控制式外部产品不等于 #44 的真实认证。
...
- main 的 `.dsh/harness` 指向 `/root/workspace/batch4-upgrade.VkSXdm/harness`，精确支持 C `3c38b1d4e8bf219750203e44b1df033ced754e92`；docs digest 为 `1cfdeaf1262f0101099ee245b9977a0ef93d56dcc631878f313dffea27adaf41`。本轮严格检查 648／0 warnings。Codex 0.149.1、Claude SDK 0.3.241／native 2.1.241 保持不变。
- `/root/workspace/deepseek-harness` 现在是上述官方 `5dda764`／0.1.5 研究目录，不再是历史 A 源，也不是 main 的准备源。现有十项未提交改动不属于本次研究，不得重置或纳入官方源码结论。历史 B `/root/workspace/pr61-fixes.7yIVdj/harness` 及其他验证工作树保留。
- `/root/workspace/pr63-source-repair.UKB4f7` 持有 C／B／固定官方旧版的独立 Git 管理目录、原指针备份与全新 A 源，不是可清理临时垃圾。四个 stash 与既有分支未动；不按旧路径描述恢复、删除或替换源码。
...
1. 先交付官方 0.1.5 源码差异、机会与迁移／重规划报告，等待用户明确保留范围及是否接受必要维护补丁；分析不授权实现或切换支持锁。
2. 只有用户恢复开发后才更新权威 Spec／Issues 并准备新的隔离候选。若选 0.1.5，先验证必需 Team 事实与 Session 3 的完整联合迁移，再做最终归档／Studio／错误冒泡验证；#44 真实认证针对最终选择的交付一次集中收口，不沿用旧基线验收冒充新版本通过。
3. PR #61／#62／#63 均已合并，不重复处理。保留原 stash、工作树、B／C 支持与历史证据；不继续旧 PIPELINE_STATE、不关闭父 Spec #18、不发外部通知或清理数据。

## 权威材料与技能

- 上一轮使用 `research`、`dsh-plugin-dev`、`codebase-design` 完成官方 0.1.5 研究。用户随后要求移除 `dsh-plugin-dev`，执行结果见顶部；本条及以下技能记录仅描述历史，不构成继续使用或恢复开发的授权。
