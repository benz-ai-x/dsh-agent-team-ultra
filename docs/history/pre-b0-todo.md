...
## Previous requested scope: pause development and reassess official Harness 0.1.5

The user previously paused development on 2026-09-09 and requested a source-backed comparison
of `/root/workspace/deepseek-harness` with implemented Ultra features and the
remaining plan. This supersedes every historical instruction below to continue
the frozen pipeline. That research turn changed local documentation only. The
current B0 authorization and checklist above supersede its development pause;
the historical full-feature pipeline and external publication remain inactive.

- [x] Freeze the comparison at official `5dda764ed3aa172535a7967b06ff95d9cbfe536a` / `0.1.5-alpha.1`; exclude and preserve its ten pre-existing dirty files.
- [x] Confirm Ultra main and origin/main at merged PR #63 `f84584def627b4af78a029af8788de73da0ad567`; the prepared supported C source remains `3c38b1d4e8`, Session 2, and strict checks pass 648 / 0 warnings.
- [x] Read live Spec #18 and #44; GitHub reports #19–#43 CLOSED, #18 and #44 OPEN. Do not reopen completed work or infer authenticated native acceptance from the previous automated gates.
- [x] Complete the [official 0.1.5 reassessment](docs/research/2026-09-09-official-015-reevaluation.md), with capability, migration, opportunity and replanning evidence; proposals remain unimplemented.
- [x] On the user's follow-up request, remove the obsolete `dsh-plugin-dev` skill activation links for Codex and Claude and replace its mandatory AGENTS rule with source/API/test verification; preserve the skill source and historical evidence.
- [x] User selected an official-first, DSH-only reduced baseline and authorized local cleanup; preserve all old data separately rather than assuming a lossless migration or external Issue-edit authority.

## Previous requested scope: complete acceptance and merge PR #63 (completed)

PR #63 was merged on 2026-09-09 as `f84584d`; main was synchronized. The following
records describe its completed acceptance, not a current instruction to merge.
...
- [ ] After the pause and target decision, perform #44 genuine authenticated native acceptance on the selected final candidate; controlled external boundaries are not this acceptance.
...
- [x] Complete #39–#43 through merged PR #63; all five Issues are closed. Their acceptance qualifies the fixed Session 2 C source, not official 0.1.5 / Session 3.
- [ ] Replan the remaining #44 acceptance after the user's current development pause and target decision; parent Spec #18 stays open.
...
- [ ] Reconcile any future plan with the live Issues after user confirmation. [PIPELINE_STATE.md](PIPELINE_STATE.md) is historical and must not resume the paused pipeline; leave Spec #18 open for the final acceptance audit.
