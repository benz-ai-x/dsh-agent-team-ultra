...
Harness 固定为 `d5eca257c2a0a21392ae8af0ee050f28358362f6`，Session 0、
...
| 丢回执、重启、同输入重放、改输入冲突、旧代际拒绝 | Ultra 在存活 turn 中重试丢失的 native 回复并恢复原回执；完整进程重启将遗留 turn 恢复为 interrupted，保留原业务提交并只结算一次中断。Harness `recovers the original native receipt...` 与 `keeps a committed message when provider retirement...` 验证旧 grant 拒绝和新 grant 恢复。 |
| 最终／失败／中断结果持久回 Lead，结算不重复 | Ultra 在线结果、failed/interrupted 文本状态、JSON／SQLite 原生终态已落盘但通知丢失后的重复 Host 重启测试；Harness settlement 重放覆盖原回执。超大 UTF-8 结果明确截断，保留完整请求限额。 |
...
修复后 2026-09-06 本地验证：`pnpm context:check:strict` 554 项／0 警告；完整
`pnpm verify` 274 项测试／22 文件、八归档实际安装、成员消息／回执重放／终止
结果、JSON／SQLite 原身份恢复、真实 Web 启动和卸载全部通过。归档断言位于
[probe-codex-continuity.mjs](../../scripts/probe-codex-continuity.mjs)。
修复后 Harness owning tests 196 项、业务源码覆盖率 100%；TS SDK 真实进程及
Python 实际单文件运行时均保留并回放新事件，原始回执／消息关联断言通过。
修复后完整 verify、Harness build、32 项 doc-sync、全 lint 均通过。
最终 Standards／Spec 审查单独执行，完成验证不代表已获审查批准。
...
Codex 冷恢复不会恢复已死亡的工具 RPC callback，也不允许历史 turn 发起新的写入。
真实 Host 存储重建后的回执重放与 Codex 的终态恢复分别验证；不得将两者表述为
向已死亡的 native 调用重新送达工具结果。进程崩溃后追加工作必须先恢复并重新授权成员。
