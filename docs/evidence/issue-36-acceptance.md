...
| --- | --- |
| Establish a complete baseline, then bounded invalidation and authoritative rereads | Host tests open the follower before reading the baseline, so an overlapping commit cannot be lost. The first frame is a complete `TeamView`; synchronous message/task bursts retain one pending invalidation bit per follower. Message page and roster keep their existing single in-flight/latest-dirty bounds. The task view now owns exactly one generation-scoped refresh cycle: all invalidations share one Promise completion and set one mutable dirty bit, at most one authority read runs, and another read begins only when an invalidation arrived during the prior read. A public `async_hooks` test holds both the initial and trailing reads while delivering 4096 invalidations in each period, observes no more than one new completion per period, and requires the final post-invalidation Host revision to publish. No stream frame or latch stores a page, body, task patch, or Client-authored authority. |
...
| Share Host commit facts and keep distinct bilingual states | The same Host follower observes a real `remoteSendMessage` acceptance/delivery and task commits. Authority reads return the exact delivered message and current task view. Production UI tests distinguish empty/loading, disconnected before baseline, stale after baseline, unavailable terminal transport, stale-CAS conflict, and retained committed data in English and Chinese; Chinese stale copy consistently uses “陈旧”. On task CAS conflict, a successful authority reload preserves the draft and advances only the next explicit Save base; a failed reload retains the real error and old base. Neither path auto-retries, and completed dependencies disappear only from blocker copy while their DAG edges remain authoritative. |
...
- Unified-retry Harness TeamAction passes 42/42. The public
  ...
  set passes 115/115. The built-library stream descriptor e2e passes 1/1.
- Harness full typecheck, focused lint, official Host/Client/Web builds, 32/32
  documentation gates, the focused Team Web replay 3/3, snapshot corpus
  ownership/header/redaction 2/2, and complete GUI regression (283 files, 3925
  passed, one existing skip) pass. No artifact was hand-edited.
- Ultra source preparation attests Harness
  `806e58873bb055e258b4623d03c9fbad41a26dc7` and docs digest
...
- The sole-unified-retry final `pnpm verify` passes on its first run with 582
  strict checks / zero warnings, production Host/Client and generated-artifact
  builds, 359 tests in 30 files, and the complete eight-archive install, Web,
  dual-runtime recovery, lifecycle, and uninstall gate. Before review round 1,
  the first formal local-gate run
