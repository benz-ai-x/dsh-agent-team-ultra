...
| [ec0f240b03](https://github.com/benz-ai-x/deepseek-harness_x/commit/ec0f240b034c11ea3edc939db59c6b09ba530436) | 从同一Host task view只把未完成前置项呈现为blocker，同时保留完整依赖边；fit-to-view以实际视口边距完整容纳并居中普通多行DAG，不改变手动zoom/pan / present only unfinished prerequisites as blockers from the same Host task view while retaining every dependency edge; fit the complete ordinary multi-row DAG inside the real viewport margin without changing manual zoom or pan | no persistent or public API change | public TeamAction blocker/watch RED/GREEN; five-row 260px viewport RED/GREEN; complete TeamAction 38/38 | same maintained branch; PR #60 sole unified retry (#34) |
| [13f5fb25d7](https://github.com/benz-ai-x/deepseek-harness_x/commit/13f5fb25d7f43e67103f176ca6f603178f86c2ef), [938adcc566](https://github.com/benz-ai-x/deepseek-harness_x/commit/938adcc566f9a6a4eddb90f8f03920684a0e28e6) | 成功conflict authority reload只为下一次显式Save推进edit base；失败保留真实错误与旧base，均不自动重试；并发删除的已选dependency仍以英中不可用选项留在草稿且可取消 / a successful conflict authority reload advances the edit base only for the next explicit Save; failure keeps the real error and old base, neither retries automatically; a concurrently deleted selected dependency remains as a bilingual unavailable draft option that can be removed | no persistent or public API change | second explicit Save, stable reload failure, and deleted dependency public UI RED/GREEN; Agent Note/README formal pairing; TeamAction 41/41 before #36 | same maintained branch; PR #60 sole unified retry (#35) |
