...
User instruction (2026-09-05): close implementation issues once development and validation pass; PR creation and reviews remain independent tasks. #19–#25 meet this condition, but GitHub closure is pending authentication (`gh issue close 19` was rejected before any state change).
...
- [ ] [A05 / #23](https://github.com/benz-ai-x/dsh-agent-team-ultra/issues/23): Codex moved to `@benz-ai-x/dsh-agent-team-codex` on `fix/23-ultra-codex-runtime`, based on A04. Full `pnpm verify` passes 506 strict checks, 199 tests and eight-archive install/boot/uninstall. Actual predecessor-to-current archive upgrades preserve Profile/member/native handle identity on JSON and SQLite, including follow-up messages, Loader removal/replacement, Web boot and residue-free uninstall. Closure, PR creation and post-submission review await GitHub CLI authentication.
- [ ] [A06 / #24](https://github.com/benz-ai-x/dsh-agent-team-ultra/issues/24): Claude Code moved to `@benz-ai-x/dsh-agent-team-claude-code` on `fix/24-ultra-claude-code-runtime`, based on A05. Full `pnpm verify` passes 554 strict checks, 213 tests and eight-archive install/boot/uninstall. Actual predecessor upgrades on JSON and SQLite preserve member/Revision/native Session identity and read-only permissions; active Query disposal, cold resume, follow-up messages, Web boot and complete uninstall pass. Closure, PR creation and post-submission review await GitHub CLI authentication.
- [ ] [A07 / #25](https://github.com/benz-ai-x/dsh-agent-team-ultra/issues/25): read-only JSON/SQLite migration audit and format/delivery ADR implemented on `fix/25-read-only-migration-audit`, based on A06. Full `pnpm verify` passes 554 strict checks, 240 tests and eight-archive install/boot/uninstall. Codex and Claude Code predecessor-to-current archive upgrades pass on JSON/SQLite with preserved identities, follow-up work and resource disposal. Phase A development and verification are complete; closure, PR creation and post-submission review await GitHub authentication. The Phase C migration is not executed and the runtime lock is unchanged.
- [ ] Continue open implementation issues #26–#44 in order after A07, preserving their dependency and integration-branch requirements. Leave Spec #18 open for the final acceptance audit.
