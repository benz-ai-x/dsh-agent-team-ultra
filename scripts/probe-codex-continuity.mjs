...
try {
  ...
  const conversationCallPrefix = queries ? 'installed-profile-conversation' : 'upgraded-profile-conversation'
  let modelLaunch
  if (phase !== 'before') {
  ...
  if (phase !== 'before') {
    ...
    if (queries) {
      ...
      assert.deepEqual(toolResult.value, { ok: false, error: {
        code: 'profile-in-use', message: 'Team member name "codex-reviewer" is already reserved',
      } })
